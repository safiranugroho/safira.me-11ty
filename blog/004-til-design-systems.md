...
This was the case for our design system components – they were primarily used as transitive dependencies by the other in-house products. There was another layer of abstraction between the design system components and the product teams, and the source code for these middle-layer components live in the same monorepo. This meant that we *couldn't* release any breaking changes without forcing them to upgrade their usages because everything in the monorepo followed the same release process. This tight coupling of the release process was one of the reasons why it took three months before that new major version went live.
